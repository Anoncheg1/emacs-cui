...
;; Set token to nil to disable token for :local:
(setq cui-restapi-con-token
      (plist-put cui-restapi-con-token :local nil))
;; Optionally, set it as default service:
...
- :sys - string - system prompt, see ~cui-block--get-sys~ in [[./cui-block.el]]
- :chat, :completion, :chain  - bool - specify main function that will handle ~C-c C-c~. For :chain see [[./cui-prompt.el]]

Note: for fast editing :sys parameter in (") double quotes with ~forward-sexp~ function set:
#+begin_src elisp :lexical t :results none :exports code :eval no
(modify-syntax-entry ?\" "\"" org-mode-syntax-table) ; make `forward-sexp' and `backward-sexp' functions jump to end of (")
#+end_src
* Debugging
...
- Search in Chinese text	https://github.com/Anoncheg1/pinyin-isearch
